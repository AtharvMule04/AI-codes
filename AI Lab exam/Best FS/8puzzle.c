...
void input_initial(int *tiles) {
    printf("Enter initial 8-puzzle state (use 0 for blank):\n");
    printf("Example goal: 1 2 3 4 5 6 7 8 0\n");
    for (int i = 0; i < N; i++) {
        scanf("%d", &tiles[i]);
    }
}
...
int heuristic(const int *tiles) {
    int h = 0;
    for (int i = 0; i < N; i++) {
        if (tiles[i] != 0 && tiles[i] != goal_state[i])
            h++;
    }
    return h;
}
...
void print_solution_path(int goal_idx) {
    ...
    printf("\nSolution found in %d moves (Best First Search):\n", length - 1);
...
void best_first_search() {
    ...
    if (goal_index == -1) {
        printf("\nNo solution found using Best First Search.\n");
    } else {
        print_solution_path(goal_index);
        printf("\nGoal Reached Successfully (via Best First Search)!\n");
    }
}
...
int main() {
    ...
    printf("\nStarting Best First Search (heuristic = misplaced tiles)...\n");
    best_first_search();
